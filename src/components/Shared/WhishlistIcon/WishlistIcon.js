...
export function WishlistIcon(props) {
  ...
  const [hasWishlist, setHasWishlist] = useState(null); // consulta si lo tiene en favoritos
  const { user } = useAuth(); // me traigo user para poder sacar el user.id

  useEffect(() => { // se cambia si se actualiza gameId
    (async () => {
      try {
  ...
  return (
    <Icon
      name={hasWishlist ? "heart" : "heart outline"} // pintado corazón o despintado
